...
static void encode_graph(Wcnf *F, vector<vector<int>> &edgeList, int nodes,
  ...
  /*
  ord*_ij = {ord_ij if i<j, -ord_ji else}
  n = |V|
  |ord*_ij| = n(n-1)

  for i = 1..n-1
    for j = 1..n-1                 row      col
      i <= j ? Var(ord_i,j+1) = (i-1)(n-1) + j
             : Var(ord_i,j)   = (i-1)(n-1) + j

  Same for O_ij, with offset = n(n-1)

  Mapping variable back:
  Var x => x > n(n-1) ? O_ij : ord*_ij

  i = x % (n-1), j = x / (n-1), j = (j>i) ? j : j+1
  ...
  */
  ...
  // cout << "\nHrad clauses of the existing edges\n";
  // finally for each edge (v_i,v_j) in E, i<j, add (O_ij or O_ji)
  // since each edge is represented once in the edgelist, i<j is redundant
  // constraint, right?
  for (auto &edge : edgeList) {
    int i = edge[0];
    int j = edge[1];
    lits.clear();
    lits.push_back((makeLit(i - (zero_offset ? 0 : 1), j - (zero_offset ? 0 : 1), n, oijs_offset)));  // O_ij
    lits.push_back((makeLit(j - (zero_offset ? 0 : 1), i - (zero_offset ? 0 : 1), n, oijs_offset)));  // O_ji
    F->addHardClause(lits);
  ...
  for (uint64_t i = wOffset; i < wOffset + n - 1; i++) {
    ...
    F->addHardClause(lits);
    F->addSoftClause(mkLit(i), 1);
  }
  F->addSoftClause(mkLit(wOffset + n - 1), 1);

  F->set_dimacs_params(newVarOffset, F->nHards() + F->nSofts(), n + 1);
  // F->printSimpStats();
  // F->printFormulaStats();
}
